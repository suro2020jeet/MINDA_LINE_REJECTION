sap.ui.define([
	...
	"sap/m/MessageToast",
	"com/minda/LineRejection/model/formatter"
], function (Controller, JSONModel, Filter, FilterOperator, Spreadsheet, MessageToast, formatter) {
	"use strict";

	return Controller.extend("com.minda.LineRejection.controller.LineRejection", {
		formatter: formatter,
		onInit: function () {
			this.getOwnerComponent().getRouter().getRoute("linerejection").attachPatternMatched(this._onProductMatched, this);
		},
		...
		_getPPMData: function (FirstDay, date) {
			...
			filter.push(new sap.ui.model.Filter("CreateDate", sap.ui.model.FilterOperator.BT, this.getOwnerComponent().getModel(
				"detailViewModel").getProperty("/fromDate"), this.getOwnerComponent().getModel(
				"detailViewModel").getProperty("/toDate")));
			this.getOwnerComponent().getModel().metadataLoaded().then(function () {
				this.getOwnerComponent().getModel().read("/LineRejectionSet", {
					filters: filter,
					success: function (oData) {
						debugger
						if (oData.results.length == 0) {
							this.getOwnerComponent().getModel("detailViewModel").setProperty("/items", []);
							this.getOwnerComponent().getModel("detailViewModel").setProperty("/tableTitle", "Rejection (0)");
						} else {
							this.getOwnerComponent().getModel("detailViewModel").setProperty("/items", oData.results);
							this.getOwnerComponent().getModel("detailViewModel").setProperty("/tableTitle", "Rejections (" + oData.results.length + ")");
						}
		...
		onPressAdvancedSearch: function () {
			if (!this._oDialog) {
				this._oDialog = sap.ui.xmlfragment("com.minda.LineRejection.fragments.AdvancedSearch", this);
